...
class MMTVAE(tf.keras.Model):
    def __init__(self, latent_dim, number_atoms,
                 global_batch_size, num_attention_heads=5,
                 **kwargs):
    ...
    def train_step(self, data):
        ...
        identity_matrix = tf.eye(400)
        identity_tensor = tf.tile(tf.expand_dims(identity_matrix, axis=0), [global_batch_size, 1, 1])
        ...
        with tf.GradientTape() as tape_rec:
            ...
            reconstruction_str = tf.linalg.set_diag(reconstruction_str, tf.ones((global_batch_size, 400)))
            one_tensor = tf.ones((global_batch_size, 400, 400))
...
def main(args):
    ...
    with strategy.scope():
        ...
        vae = MMTVAE(latent_dim, number_atoms, global_batch_size=BATCH_SIZE)
